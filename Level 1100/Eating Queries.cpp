...
void SolveProblem()
{
    ...
    ll sum = 0;
    vector<ll>sugar(n);
    vector<ll>prefixMaxSum;
    prefixMaxSum.push_back(-1);
    vector<ll>queries(q);
    
    for (auto& i : sugar) cin >> i;
    sortD(sugar);

    for (ll i = 0; i < n; i++)
    {
        sum += sugar[i];
        prefixMaxSum.push_back(sum);
    }

    for (auto& j : queries) cin >> j;
    for (ll i = 0; i < q; i++)
    {
        cout << FirstGreaterElement(prefixMaxSum, queries[i]) << '\n';
    }
}
