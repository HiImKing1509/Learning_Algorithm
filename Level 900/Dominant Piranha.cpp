...
void SolveProblem()
{
        ...
        ll maxArr = *max_element(a.begin(), a.end());
        ll numMax = count(a.begin(), a.end(), maxArr);

        ll index = -1;
        if (numMax == n) index = -1;
        else if (a[0] == maxArr) index = 1;
        else if (a.back() == maxArr) index = n;
        for (ll i = 1; i < n - 1; i++)
                if (a[i] == maxArr && (a[i - 1] != maxArr || a[i + 1] != maxArr))
                        index = i + 1;
